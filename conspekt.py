...
Он не содержит все файлы репозитория (если только это не первый коммит). Таким образом, каждый коммит — это небольшой кусочек истории репозитория, основанный на предыдущем коммите. 
...
Это что бы знать куда отправлять код
...
touch hello.html
git init
git add hello.html (Добавить/создать файл с кодом)
git commit -m "Initial Commit" (Изменить)
git status
<h1>Hello, World!</h1>
git add hello.html
git status
